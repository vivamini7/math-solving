...
def main(question, times, n, min_voters, max_voters):
    ...
    try:
        ...
        while tie or voter_count < min_voters:
            ...
            information += f"Initial conditions: {conditions}\n"
            information += f"Objectives: {objectives}\n"
            ...
            for time in range(times): # Try to reduce the LLM queries.
                information += f"\n# {voter_count} Thinker is thinking new thoughts (iteration {time+1})...\n"
                unchecked_conditions = Think_thoughts(conditions, objectives)
                information += f"Unchecked conditions: {unchecked_conditions}\n"
                checked_conditions = []
                print(information)
                ...
                information += f"Checked conditions: {checked_conditions}\n"
                conditions = conditions + checked_conditions
                if_got_answer = check_if_got_answer(conditions, objectives, 1)
                if if_got_answer:
                    information += f"Answer found in iteration {time+1}\n"
                    break
                print(information)
            ...
            information += f"Final answer calculation: {final_answer}\n"
            print(information)
            # Achieve one potential answer
        ...
        information += f"Error processing file: {e}\n"
        return information
